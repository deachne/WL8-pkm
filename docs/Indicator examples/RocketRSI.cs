...
namespace WealthLab.TASC
{
    ...
    public class RocketRSI : IndicatorBase
    {
        ...
        public RocketRSI(TimeSeries source, int smoothLength, int rsiLength)
        {
            Parameters[0].Value = source;
            Parameters[1].Value = smoothLength;
        ...
        //static method
        public static RocketRSI Series(TimeSeries source, int smoothLength, int rsiLength)
        {
            string key = CacheKey("RocketRSI", smoothLength, rsiLength);
            if (source.Cache.ContainsKey(key))
                return (RocketRSI)source.Cache[key];
            RocketRSI rr = new RocketRSI(source, smoothLength, rsiLength);
            source.Cache[key] = rr;
            return rr;
        }
        ...
        protected override void GenerateParameters()
        {
            ...
            AddParameter("RSI Period", ParameterType.Int32, 10);
        }
    }
}
