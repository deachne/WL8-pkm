...
namespace WealthLab.TASC
{
    public class SuperSmoother : IndicatorBase
    {
        ...
        //static method
        public static SuperSmoother Series(TimeSeries source)
        {
            string key = CacheKey("SuperSmoother");
            if (source.Cache.ContainsKey(key))
                return (SuperSmoother)source.Cache[key];
            SuperSmoother ss = new SuperSmoother(source);
            source.Cache[key] = ss;
            return ss;
        }
