...
namespace WealthLab.TASC
{
    ...
    public class FRAMA : IndicatorBase
    {
        ...
        //static method
        public static FRAMA Series(TimeSeries source, int period, double k)
        {
            string key = CacheKey("FRAMA", period, k);
            if (source.Cache.ContainsKey(key))
                return (FRAMA)source.Cache[key];
            FRAMA frama = new FRAMA(source, period, k);
            source.Cache[key] = frama;
            return frama;
        }
        ...
        protected override void GenerateParameters()
        {
            ...
            AddParameter("K", ParameterType.Double, 4.6);
        }
